...
- `technical_reference_consistency_engine` 已启动落地，开始把技术章节中的错位标准引用和检测证明形式限制收束为主题问题，并已在 BK 电子仪器仪表采购项目样本中验证出：
  - 技术要求中混入与标的不匹配的标准引用和检测报告形式限制
- `commercial_burden_analyzer` 已启动落地，开始把担保、备用金、异常交货期限、验收费转嫁和责任负担收束为商务主问题，并已在 BK 电子仪器仪表采购项目样本中验证出：
