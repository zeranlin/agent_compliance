...
- 已新增 [agent-incubation-and-distillation-design.md](https://github.com/zeranlin/agent_compliance/blob/main/docs/design-docs/agent-incubation-and-distillation-design.md)，并在 `src/agent_compliance/incubator/` 落地第一版生命周期骨架，用于把“业务需求定义 -> 样例驱动 -> 强通用智能体设计 -> 本地目标智能体生成 -> 对照验证 -> 持续蒸馏 -> 固化发布”沉淀成后续新智能体的标准生成方法
