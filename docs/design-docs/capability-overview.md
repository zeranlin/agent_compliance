...
- 以真实文件人工审查 vs 代码审查差异为核心驱动
- 优先补齐查点覆盖率、误报抑制、结构化输入质量、正式意见输出和复审一致性
- 近期已补入农林牧渔/造林绿化场景识别，并压制“是否属于政务信息系统项目：否”这类否定型项目属性对信息化路径的误触发

当前暂不作为新增建设主线的方向：
- `incubator` 新功能继续扩张
- `budget_demand` 等新智能体产品线增强
- 与正式审查主链无直接关系的页面扩张和外围控制台建设
...
当前工厂已补入“第一层需求定义”独立页面：
- `/incubator/definition`
- 用于把业务方“我要一个什么智能体”的自然语言需求，先分析成建议处理流程和待补充问题，再整理成可进入后续样例准备和蓝图设计的需求定义确认稿
- 当前至少会生成：
  - 产品定义
  - 能力边界
  - 第一版目标
...
当前工厂也已完成第二个真实目标的最小孵化验证：
- 已新增 `政府采购四类专项检查智能体` 蓝图
...
- `pdf` 当前已补入专门解析链路，优先使用本地 PDF 提取器而不是继续统一走 `textutil`，并在主解析结果不足时自动兜底，避免上传 PDF 后正文为空或极短
- 对表格型 PDF，当前还会在正文提取后优先合并常见采购表格碎裂标签，并在 `section_splitter` 中压制百分比、金额、表头行等伪章节标题，减少后续审查的结构污染
